...
**Database:** MySQL. **Local development does NOT use `localhost`** (corrected 2026-09-24, see §97): `bhav-bhakti-be/.env`'s `DB_HOST` points at a Railway-hosted database. Until 2026-09-24 it was actually pointing at **production's** database (`shinkansen.proxy.rlwy.net`) the whole time, while still saying `NODE_ENV=development`; it's now repointed at the Railway development environment's own database (`metro.proxy.rlwy.net`, see §96). Always check `DB_HOST`, never `NODE_ENV`, to know which database a local script will hit — database-writing scripts now enforce this via `scripts/confirm-db-target.js` (§97). The `dev`/`start` npm scripts run plain `nodemon`/`node`, no Docker. Note: Docker files (`Dockerfile`, `docker-compose.yml`, `docker-compose.cost-efficient.yml`) still exist in the repo and are referenced by `DEPLOYMENT_GUIDE.md` as a planned AWS EC2 + Docker **production** deployment path — they weren't deleted, just unused for local dev. Don't assume "Docker was abandoned" means the files are gone.
- **19** Sequelize models in `src/models/` (Category, DailyHoroscope, Deity, Feed, FeedDownload, FeedLike, FeedMedia, FeedShare, FeedTag, HoroscopeFeedback, HoroscopeReadHistory, LoginHistory, OTPLog, Payment, Subscription, SubscriptionEvent, User, UserProfile, UserZodiac). The 4 Quiz models were removed 2026-09-25 (§101).
- **46** migration files in `src/database/migrations/` (as of 2026-09-25).
- **Hosted on Railway since 2026-08-25 (§64)** — backend + MySQL live on Railway; production and a separate development environment (§96) each have their own database. (This bullet previously said no cloud DB hosting existed — stale since §64.)
...
**Deities: 20/20 rows fully bilingual** (live DB query, 2026-07-19).
...
**Mantra quiz: removed entirely on 2026-09-25 (§101)** — its recommendation output was never built on either end.
...
**Phase 1:** back-button fixed (explicit navigate-to-Home, same pattern already used for `audio-player.tsx` in §27, since `mantras.tsx` is reachable both via tab-bar switch and via real push navigation with back-history); the "Find Your Perfect Mantra" quiz-promo card hidden (the whole quiz feature was later removed outright, see §101).
...
A new Railway **development environment** now exists for the backend — own separate MySQL database, deployed independently from production off a new `bhav-bhakti-be` `develop` branch — for testing changes before they go live, rather than testing against real production data/traffic. **Not yet wired up: the frontend's `.dev` app still points at production's backend**, not this new dev environment — switching it over is planned for later, not done this session.

**First batch of security-audit fixes — DONE, CONFIRMED WORKING, live in production on both repos.** Backend: TLS enforced on the database connection, HSTS enabled, and inconsistent error-message leakage across controllers (raw exception details reaching the client in every environment) fixed via a new centralized `ResponseHandler.errorSafe()` helper, swept across the whole `controllers/` directory. Frontend: JWT/PII data (tokens, phone numbers, full user/request objects) no longer logged unconditionally across `authStore.ts`, `useAuth.tsx`, `apiClient.ts`, `secureStorage.ts` — confirmed clean via a real on-device logcat capture (login + OTP verification). All fixes verified with real requests against both the new dev environment and live production, then promoted `dev`/`develop` → `production` → `master` on both repos. Full detail lives in `Bhav_Bhakti_Security_Audit_Plan.md` — this is a pointer/summary, not a repeat of it.

## 97. 2026-09-24 session (continued) — Security audit Group 2 (Access Control): DONE, CONFIRMED WORKING, live in production

**DONE, CONFIRMED WORKING, live in production** — `bhav-bhakti-be` `develop` merged into `main` (`7698adb`), auto-deployed by Railway. Verified by a 31-check smoke test run directly against production (not just dev), all passing, nothing written. Backend-only — the app never called any of the removed endpoints, so no frontend changes were needed.

**What shipped:** `requireAdmin` on all `/categories/admin` routes, all 5 upload routes (including `/presigned-url`), and file delete (`DELETE /upload/media/:key`); `authenticate` added to `/analytics/global-stats`; Facebook OAuth fully removed (routes, controller, service, env vars — the `users.facebook_id` column was kept, no migration); the fake, forgeable `admin.auth.middleware.js` deleted; `POST /profile/photo` removed and a real, live vulnerability closed — any logged-in user could set `profilePicture` to any content file's URL, then `DELETE /profile/photo` deleted that file from storage (it now only clears the DB reference, and neither profile-update endpoint accepts `profilePicture`). Two pre-existing bugs fixed along the way: `GET /categories/admin` was unreachable (caught first by `GET /:categoryId`), and `GET /categories/:id/feeds` always returned a 500 (it queried non-existent columns).

**New safety mechanism:** `scripts/confirm-db-target.js` — before `import:feeds`, `create-categories-from-csv.js`, or `create-deities-from-csv.js` touches the database, it shows the target labeled PRODUCTION/DEVELOPMENT/LOCAL/UNKNOWN, derived from the real `DB_HOST` (never `NODE_ENV`, which proved unreliable this session — see §2), and requires typing that label; it refuses outright when run non-interactively. Its host-to-label map needs updating if Railway ever regenerates a proxy hostname.

**Significant finding, fixed:** production's `JWT_SECRET` was still the `.env.example` placeholder value — anyone guessing it could forge a login token for any user. Rotated to a real random secret in both Railway environments.

**Still open (brief):** `role = 'admin'` not set on the founder's production account — deliberately deferred, only needed for the unused manual Postman/curl upload path (the CSV importer bypasses the API entirely); dev database password rotation declined by the founder; security audit Groups 3–9 not yet started. Full detail lives in `Bhav_Bhakti_Security_Audit_Plan.md`.

## 98. 2026-09-24 session (continued) — Security audit Group 3 (Rate Limiting): DONE, CONFIRMED WORKING, live in production

**DONE, CONFIRMED WORKING, live in production** — `bhav-bhakti-be` `develop` merged into `main` (`8fee297`), auto-deployed by Railway. Every limit was triggered for real on the dev environment; production got a light smoke test only (counters decrement correctly, `/profile` limiter works, malformed JSON returns 400) — login/webhook limits deliberately not triggered on production. Backend-only.

**Five limits now active** (`express-rate-limit`, all in `src/middlewares/rateLimit.middleware.js`, each value a single named constant at the top of the file):
- Login (`POST /auth/firebase/verify`): 20/min **and** 75/hour per IP.
- Payment initiation (`POST /subscription/create`): 5/hour per authenticated user (runs after `authenticate`, keyed on `req.user.id`).
- Razorpay webhook: 100/min per IP — a flood backstop only.
- General content API (feed, horoscope, categories, deities, feature-flags): one shared 300/min per-IP budget, mounted in `src/routes/index.js` so it runs before any route-level `authenticate`/`requireAdmin`.
- Pre-existing `/profile/*` limiters (20 writes/15 min, 30 reads/min) — unchanged.

Every 429 is `{ success: false, message }`, identical to any other API error. The new limiters log each rejection as a `[rate-limit]` line (limiter, route, IP or user ID, timestamp) — search Railway logs for it if a real user reports being blocked.

**Login and content limits are deliberately MORE generous than the audit plan's original item 16 draft (10/min + 30/hour, 100/min)** — a founder decision, not an oversight: Jio/Airtel put many real users behind one shared public IP (CGNAT), which per-IP limits can't tell apart from abuse.

**Real bug found and fixed: the app wasn't seeing real client IPs.** Railway puts two relays in front of the app (public edge, then an internal proxy), so `src/app.js` now sets `trust proxy` to `2`. With the wrong value, per-IP limits either never triggered (split across Railway's several edge IPs) or lumped users together. This also fixed a pre-existing production issue: the `/profile/*` limiter had been counting large groups of real users against a shared internal relay address. Side effect: request logs (`morgan`) now show real client IPs. Railway's edge discards client-supplied `X-Forwarded-For`, so the IP can't be spoofed. **If a CDN/proxy is ever put in front of `api.orivori.com`, this hop count must change.**

**Incidental fix:** malformed JSON request bodies now return 400 instead of 500 (`src/middlewares/error.middleware.js`).

**Known limitation:** counts live in server memory — they reset on every deploy/restart, and if the backend ever runs multiple instances, each keeps its own separate count (a shared store like Redis would then be needed).

## 99. 2026-09-24 session (continued) — Security audit Group 5 (Automated Database Backups, item 23): DONE, CONFIRMED WORKING, live in production

**DONE, CONFIRMED WORKING, live in production** — `bhav-bhakti-be` `develop` merged into `main` (`aae14c3`). A dedicated Railway cron service (production only, auto-deploy off, set up manually by the founder) runs `scripts/backup-database.js` daily at 3am IST (`30 21 * * *` UTC). Each run: `mysqldump` → gzip → Firebase Storage at `backups/YYYY-MM-DD-HHmm.sql.gz` (India time) → retention cleanup. Every run ends with one `[db-backup] SUCCESS …` or `[db-backup] FAILED stage=… error=…` log line; failures exit 1. First live production run: **~509 KB compressed, 5.9 seconds.**

**Separate Docker image (`Dockerfile.backup`, selected via `RAILWAY_DOCKERFILE_PATH`)**, built on the official `mysql:9.4` image plus Node. The main backend's Alpine-based image can't be used: Alpine only offers MariaDB's `mysqldump`, which can't log in to MySQL 9.x (`caching_sha2_password` plugin missing). The main API's `Dockerfile` is untouched. **If Railway's MySQL is ever upgraded, bump the `mysql:` tag to match.**

**Private network only:** connects via the database's private address (`${{MySQL.MYSQLHOST}}`, e.g. `mysql.railway.internal`) and refuses to run against the public `*.proxy.rlwy.net` address. The 5.9s production run (vs ~97s when tested from outside Railway) confirms the private path is really in use.

**Retention (`src/utils/backupRetention.util.js`, unit-tested):** keeps the newest backup from each of the last 7 days that have one, plus the newest from each of the 4 weeks before that — at most ~11 files, about a month of history. Count-based, not calendar-age-based, so a gap in runs never wipes out old backups as "too old." Only deletes after a successful upload; never touches files that don't match the naming pattern (safe for manual one-off backups in the same folder).

**Real finding, fixed before any real backup was uploaded:** the Firebase Storage bucket allowed anyone to list and download everything with no login — a `backups/` folder of full database dumps would have been public. `storage.rules` now excludes `backups/` from public reads while all existing content (`audio/`, `Wallpapers/`, `other/`, `FCMImages/`) stays public; verified in the Firebase emulator and against the live bucket. Published by the founder via the Firebase Console — the repo's `storage.rules` matches what's live. Any future Storage rules change must keep the `folder != 'backups'` exclusion.

Restore was verified during dev testing: a backup loaded into a fresh MySQL 9.4 with 0 row-count differences and Hindi text intact.

## 100. 2026-09-24 session (continued) — Security audit Group 9 (Adversarial Testing, Phase 2.5): DONE — 4 findings, 2 fixed and live in production, 2 deliberately deferred

**10 real attack attempts against the dev environment** (not code review): cross-account reads/writes and privilege escalation via profile updates; expired, tampered, `alg:none` and wrong-secret JWTs; login-token reuse; non-admin access to `/categories/admin` and all upload/delete-media routes; forged and replayed Razorpay webhooks; oversized/negative/non-numeric page sizes; a 600-request concurrent burst on unauthenticated endpoints (limiter held at exactly 300/min). All correctly rejected except these 4 findings. Premium/`isPremium` checks skipped — Group 4 territory, not built yet.

**Fixed, live in production (merge `1ca0f5e`):**
- `POST /feed` was wide open — no login at all (the §3 "admin web interface" leftover). Now `authenticate, requireAdmin`. The CSV import scripts write via Sequelize directly and are unaffected (checked, not assumed).
- `GET /categories/:id/feeds` accepted unbounded page sizes and returned 500 on negative/non-numeric ones. Now the same 1–100 `limit` / non-negative `offset` validator as the feed routes → clean 400.

**Deliberately NOT fixed — founder decision for MVP scope, not an oversight:**
- **Firebase login-token reuse:** the same Firebase ID token is accepted repeatedly within its 1-hour validity, each time opening a new 10-day backend session. Fix when picked up: reject tokens whose `auth_time` is more than a few minutes old.
- **Razorpay webhook replay with a new event ID:** duplicate detection keys on the unsigned `X-Razorpay-Event-Id` header, so a captured signed body re-sent with a new ID is processed again. Confirmed no money or premium-time impact (payments are keyed on Razorpay's unique payment ID; handlers write absolute values) — worst case, a stale status (e.g. an old "cancelled") gets reapplied. Requires an authentic signed webhook body first. Fix when picked up: also dedupe on a hash of the raw body and/or ignore events older than the last one applied.

**Minor loose ends, not fixed, low priority:** `POST /feed` still hardcodes user 1 as the owner of every new feed regardless of who's logged in (`feed.controller.js`); `sortBy`/`sortOrder` on `GET /categories/:id/feeds` aren't validated (Sequelize quotes them, so not an injection risk — just a possible 500 on garbage input).

**Testing note:** the dev database has no admin users — admin-path tests temporarily set a dev test user's `role` to `admin` and reverted it. The dev environment's `RAZORPAY_WEBHOOK_SECRET` differs from the local `.env`'s, so signed-webhook tests ran on a local server against the dev database.
...
## 102. 2026-09-26 session — Feeds schema redesign: Phases 1–3 DONE on develop/dev, NOT in production

**Read `D:\bhav_bhakti_Claude_docs\Bhav_Bhakti_Feeds_Schema_Redesign_Plan.md` first for anything touching feeds, feed_media, tags, categories or the CSV importer** — it is the source of truth; this is only a pointer. Phases 1–3 are built, migrated on the dev database and live on `develop`/`dev` (backend `aa94797`, `8b259dc`, `536b9cc`; frontend `5a13359`). `categories` is gone. `feeds.label` is replaced by a many-to-many `tags`/`feed_tags` system — `feeds.label` still exists but is no longer read or written, and a compatibility shim still returns `label` in API responses. `feed_media` is merged into `feeds`, which now store Firebase Storage **paths** (the backend builds full URLs per response); `caption` became bilingual `subtitle`, and a shim still returns the old one-item `media[]` array and `caption`. The importer now takes a single sheet (template: `bhav-bhakti-be/Content Upload Sheet - Feed TEMPLATE.csv`) and requires `id` and `url` on every row.

**Still pending (as of this entry — the app update and the `label` removal have since been done on dev, see §103):** the app hasn't been updated to read `tags` or the new fields directly — it relies entirely on the shims — and none of this has been migrated to production (no production database, backend `main` or app `production` changes). Migrations were run against the dev database with the dev-scoped Railway token (`railway run --service MySQL`), never via `.env`.

## 103. 2026-09-26 session (continued) — Feeds schema redesign: Phases 2 and 3 complete on dev, `label` fully retired — NOT in production

**Full detail lives in `D:\bhav_bhakti_Claude_docs\Bhav_Bhakti_Feeds_Schema_Redesign_Plan.md`** — this is a pointer/summary. Everything below is on `dev`/`develop` and the dev database only; nothing has touched `main`, `production`, `master` or the production database.

- **App reads the new fields directly** (`bhav-bhakti-fe` `052761c`): `tags`, `subtitle` (now shown in the current language), `url`, `mediaType`, thumbnails and `duration`, instead of the old `label`/`media[]`/`caption` shim. Mantra cards show one pill per mood tag; the mood tiles filter with `tags=`. New helpers in `src/utils/feedFields.ts`. Downloaded/shared files are unchanged.
- **New `excludeTagGroup` filter** (`bhav-bhakti-be` `6634b8c`) on `GET /feed`, `/feed/trending` and `/feed/user/liked`: returns feeds with no tag from the given group. Validated against the `tags` table, so a typo is a 400. `tags=` inclusion now also works on trending and liked, which previously ignored it.
- **Wallpapers tab switched from `label=none` to `excludeTagGroup=occasion`** (`bhav-bhakti-fe` `9573ce8`). A grep confirmed the app no longer sends or reads `label` anywhere.
- **`label` fully dropped on dev** (`bhav-bhakti-be` `f158805`, migration `20260926000006-drop-feeds-label`, backup in `archive/feeds-label-column-backup-2026-09-26.json`): column, filter code and response field are gone. Any request that still sends a `label` query param gets a **400** naming `tags`/`excludeTagGroup` — deliberately not ignored, since ignoring `label=none` would silently widen an old build's Wallpapers tab. The migration's rollback was tested for real on dev (31/31 labels restored, then re-applied).
- **Still returned, not yet removed:** the backend's `media[]`/`caption` response shim (the app no longer reads it) — Phase 3's "drop the shim later" cleanup.
