...
- General content API (feed, horoscope, categories, deities, quiz, feature-flags): one shared 300/min per-IP budget, mounted in `src/routes/index.js` so it runs before any route-level `authenticate`/`requireAdmin`.
