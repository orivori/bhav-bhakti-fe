...
export interface UseMantraFeedResult {
  feeds: Feed[];
  isLoading: boolean;
...
// Mirrors useWallpaperFeed/useRingtones exactly (Mantra Explorer redesign,
// CLAUDE.md §56 Phase 2): trending (a ranking view) and a deity selection (a
...
export function useMantraFeed(filter: DeityFilterSelection): UseMantraFeedResult {
  ...
  const loadFeeds = useCallback(async (cursor?: string, refresh = false) => {
    try {
      ...
      console.error('❌ Error loading mantra feed:', err);
  ...
  // result sets, not pages of one query, so switching between them must not
  // append onto the previous list.
  useEffect(() => {
    setFeeds([]);
    setHasMore(true);
